...
def save_savedmodel(estimator, serving_dir, seq_length, is_tpu_estimator):
    """
       Convert the checkpoint files to pb files.
       If the model is trained, it will convert directly.
       Otherwise, it will train the model first, recording as checkpoint files, then convert them to pb files.
    """
    feature_map = {
    ...
    print("Save saved_model(pb files).")
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    saved_model_dir = FLAGS.output_dir+'pb_file/'
    save_savedmodel(estimator, serving_dir=saved_model_dir, seq_length=128, is_tpu_estimator=False)
